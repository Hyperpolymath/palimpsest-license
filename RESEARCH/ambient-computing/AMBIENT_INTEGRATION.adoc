...
*Status:* FIRST PASS / RESEARCH *Purpose:* Palimpsest in
...
As computing becomes ambient—embedded in environments, wearables, IoT
devices, smart spaces—the boundaries of "`using`" creative work blur. A
song playing in a smart space, an image displayed on ambient screens, a
poem read by a home assistant: all involve complex chains of consent and
context that current licensing frameworks ignore.
...
|Smart Home |Background media, ambient displays |Who consents in shared
spaces?

|Wearables |Personal audio, AR overlays |Micro-interactions, passive
consumption

|Smart Cities |Public displays, environmental media |Public space,
collective presence

|Automotive |In-vehicle entertainment |Driver vs. passenger consent

|Healthcare |Ambient assistance, therapeutic media |Patient capacity,
caregiver role

|Retail |Background music, digital signage |Employee vs. customer
exposure

|Education |Smart classrooms, learning environments |Minor consent,
...
Traditional consent models assume: - Discrete usage events - Single
identifiable user - Conscious choice to engage - Clear start/end of use

Ambient computing violates all of these: - Continuous background
presence - Multiple people in space - Passive exposure - Always-on
environment
...
Consent checking must happen at the edge for: - Latency (real-time
ambient environments) - Privacy (minimal data transmission) - Resilience
(offline operation)
...
[arabic]
. *Default to restriction:* If unsure, don’t play
. *Highest restriction wins:* Multi-user → most restrictive consent
applies
. *Graceful degradation:* If consent unclear, fall back to safer content
. *Attribution persistence:* Even ambient use requires findable
attribution
. *Context preservation:* Ambient doesn’t mean decontextualized
. *Exit possible:* Opt-out must be meaningful and accessible
. *Transparency:* What’s playing, why, and under what terms
. *Accountability:* Clear responsibility chain for violations
