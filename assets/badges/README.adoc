...
This directory contains badge variants for the Palimpsest License in
multiple formats and styles. These badges can be used on websites,
documentation, GitHub repositories, and other platforms to indicate that
a work is licensed under the Palimpsest License.
...
[arabic]
. *Compact* (`+badge-compact.svg+`) - 90×20px
* Minimal design for tight spaces
* Text: "`Palimpsest`"
. *Standard* (`+badge-standard.svg+`) - 128×32px
* Default badge for most uses
* Text: "`Palimpsest v1.0`"
. *Large* (`+badge-large.svg+`) - 180×40px
...
[arabic]
. *Light* (`+badge-light.svg+`) - White background, optimised for light
themes
. *Dark* (`+badge-dark.svg+`) - Dark background, optimised for dark
themes
. *Blue* (`+badge-blue.svg+`) - Blue gradient background
. *Green* (`+badge-green.svg+`) - Green gradient background
. *Flat* (`+badge-flat.svg+`) - Solid colour, no border
. *Text-Only* (`+badge-text-only.svg+`) - No icon, text centred

==== Adaptive (Light/Dark)

The *Standard*, *Compact*, and *Large* badges include CSS media queries
that automatically adapt to the user’s colour scheme preference:
...
For websites and documentation, use SVG directly for best quality and
...
[arabic]
. Copy an existing SVG badge
. Modify the colours, text, or layout
...
Host badges in your repository and reference via
...
These badge designs are part of the Palimpsest License project. You are
free to use, modify, and distribute these badges when indicating that
your work is licensed under the Palimpsest License.

The badges themselves are provided under CC0 (public domain) to
facilitate adoption, but their use should accurately represent that your
work is licensed under the Palimpsest License.

=== See Also

* `+embed/license-blocks/+` - Full licence blocks with HTML/Markdown
snippets
...
*Last updated:* 2026-01-19 *Maintainer:* Palimpsest Stewardship Council
