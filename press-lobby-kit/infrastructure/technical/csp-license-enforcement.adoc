...
*Audience:* Web Developers, System Administrators, Security Engineers,
CDN Operators
...
This document specifies how Content-Security-Policy (CSP) headers can be
leveraged to enforce Palimpsest License requirements at the browser
level. By extending CSP directives and introducing custom policies, web
servers can prevent unauthorised use of licensed content whilst
maintaining compatibility with existing web security infrastructure.
...
* *Prevent content scraping:* Block unauthorised embedding and
extraction
* *Control derivative works:* Restrict how content can be transformed
* *Enforce attribution:* Require metadata preservation in client-side
operations
* *Block AI scrapers:* Prevent interpretive AI systems from processing
content
...
[width="100%",cols="29%,48%,23%",options="header",]
|===
|Directive |Use for Licensing |Example
|`+default-src+` |Restrict content loading |`+default-src 'self'+`

|`+img-src+` |Control image embedding
|`+img-src 'self' https://trusted.org+`

|`+frame-ancestors+` |Prevent unauthorised framing
|`+frame-ancestors 'none'+`

|`+report-uri+` |Log violations
|`+report-uri /license-violation-report+`

|`+report-to+` |Modern violation reporting
|`+report-to license-endpoint+`
|===
...
[arabic]
. *Missing frame-ancestors:* Content can be embedded without protection
. *Weak report-uri:* Violations not properly logged
. *Client-side only:* Relies on browser compliance
...
Content-Security-Policy provides a powerful, browser-enforced mechanism
for implementing Palimpsest License requirements. By extending CSP with
custom directives and leveraging existing security infrastructure, we
can protect creative works at the client layer whilst maintaining
...
[arabic]
. Deploy basic CSP with reporting
. Add Palimpsest custom directives
. Implement client-side enforcement library
. Monitor violations and adjust policies
. Submit W3C proposal for standardisation
...
Questions and feedback: - Palimpsest CSP Working Group - Email:
csp@palimpsest.example - GitHub: github.com/palimpsest-license/csp

'''''

_This specification is part of the Palimpsest License infrastructure
initiative. CSP implementation examples provided under Palimpsest
License v0.4._
