...
*Version:* 1.0 *Date:* 2025-11-22 *Status:* Draft for Discussion
*Audience:* Web Developers, System Administrators, CDN Operators, Proxy
Administrators
...
This document specifies custom HTTP headers for declaring and
propagating Palimpsest License metadata across web infrastructure. By
implementing standardised license headers, content providers can ensure
licensing requirements persist through the entire HTTP request-response
cycle, from origin server through CDNs, proxies, and caches to end
clients.
...
* *Request-response symmetry:* Both clients and servers can signal
licensing
...
[arabic]
. *Non-invasive:* Don’t interfere with existing headers
. *Namespaced:* Use `+X-Palimpsest-+` prefix (or `+Palimpsest-+` when
standardised)
. *Structured:* Machine-readable and parseable
. *Extensible:* Support future license versions and features
. *Cacheable:* Work with HTTP caching mechanisms
...
*Usage:* - MUST be present on all Palimpsest-licensed content - SHOULD
be included in both request and response - MUST be preserved through
...
*Values:* - `+required+`: Explicit consent must be obtained before AI
processing - `+granted+`: Consent has been granted (with conditions) -
...
*Usage:* - Proxies SHOULD block known AI scrapers when value is
`+denied+` or `+required+` - CDNs SHOULD log AI access attempts for
compliance auditing - Clients SHOULD respect this header when processing
content
...
*Usage:* - MUST be an absolute URI (RFC 3986) - SHOULD point to JSON-LD
document - MAY include content hash parameter for integrity verification
...
*Types:* - `+required+`: Standard attribution (name, link) -
`+symbolic+`: Emotional/cultural context required - `+lineage+`: Full
...
*Values:* - `+unrestricted+`: Allow derivatives with attribution -
`+restricted+`: Allow only specified transformations - `+prohibited+`:
...
*Algorithms:* - `+sha3-512+`: SHA-3 with 512-bit output - `+blake3+`:
BLAKE3 (quantum-resistant) - `+sha256+`: SHA-256 (legacy support)
...
*Usage:* - SHOULD be calculated on complete response body - MUST be
recalculated if content is modified - MAY be used for integrity
...
*Usage:* - MUST be globally unique - SHOULD use UUID format (RFC 4122) -
...
*Usage:* - Uses HTTP-date format (RFC 7231) - Value `+never+` indicates
perpetual licensing - Caches SHOULD revalidate after expiry
...
*Usage:* - Generated per request - Logged throughout infrastructure -
...
*Usage:* - Single header containing all license metadata - Reduces
header overhead - Easier for middleware to parse
...
*Capabilities:* - `+metadata-preservation+`: Client preserves metadata -
`+attribution-display+`: Client displays attribution -
`+ni-consent-aware+`: Client respects NI consent - `+lineage-tracking+`:
Client tracks emotional lineage
...
*Values:* - `+human-reading+`: Direct human consumption - `+archival+`:
Long-term preservation - `+ai-training+`: Training AI models (requires
consent) - `+ai-inference+`: AI interpretation (requires consent) -
`+derivative-creation+`: Creating derivative works - `+analysis+`:
Research/analysis
...
*Usage:* - Server MAY deny requests based on purpose - Useful for audit
logs and compliance - Enables purpose-based access control
...
HTTP headers provide a straightforward, universally compatible mechanism
for declaring and propagating Palimpsest License metadata. By
implementing these standardised headers, content providers can ensure
licensing requirements are visible and enforceable throughout the HTTP
infrastructure.
...
[arabic]
. Implement core headers (License, Metadata-URL)
. Add NI consent and attribution headers
. Configure logging and monitoring
. Deploy AI scraper blocking
. Integrate with CDN and caching layers
...
Questions and feedback: - Palimpsest HTTP Working Group - Email:
http@palimpsest.example - GitHub:
github.com/palimpsest-license/http-headers

'''''

_This specification is part of the Palimpsest License infrastructure
initiative. Implementation examples provided under Palimpsest License
v0.4._
