...
. link:#for-creators[For Creators]
. link:#for-developers[For Developers]
. link:#for-ai-system-operators[For AI System Operators]
. link:#for-platform-providers[For Platform Providers]
...
The Creative Works Consent Framework (CWCF) is a web standard that
enables creators to express machine-readable consent for AI and
automated systems accessing their creative works. It provides:
...
* *Creators*: Authors, artists, musicians, journalists wanting to
protect their work
* *Developers*: Web developers, CMS creators building consent support
* *AI operators*: Companies building AI systems that need consent
verification
* *Platform providers*: Hosting services, CDNs implementing consent
enforcement
...
* *"`conditional`"*: Permit if requirements met (attribution, payment,
etc.)
...
[arabic]
. Download: https://wordpress.org/plugins/cwcf-consent/
. Activate plugin
. Navigate to Settings → CWCF Consent
. Configure consent preferences via UI
. Plugin automatically serves declaration at well-known URI
...
[arabic]
. Edit your `+creative-works-consent.json+` file
. Update version number (recommended):
+
[source,json]
----
{
...
. Notify AI systems if possible (via registry)
...
AI systems should honour revocations within the notice period and cease
using the work for training new models.
...
* *AI Training Monitor*: Alerts when visiting sites that allow/deny AI
training
...
*Solutions*: * Verify file is in correct location * Check web server
configuration allows `+.well-known+` directory * Ensure no URL rewriting
conflicts * Test with
`+curl https://yoursite.com/.well-known/creative-works-consent.json+`
...
*Solutions*: * Validate JSON syntax at jsonlint.com * Check `+@context+`
URL is correct * Ensure all required fields present (`+@type+`,
`+consent+`) * Verify enum values match specification

===== 9.1.4. Caching Issues

*Problem*: Changes to consent declaration not appearing

*Solutions*: * Clear CDN cache * Check `+Cache-Control+` headers * Use
cache-busting query parameters for testing * Verify no browser extension
caching
...
* *IRTF Draft*:
https://datatracker.ietf.org/doc/draft-creative-works-consent/
* *W3C CG Charter*: https://www.w3.org/community/creative-works-consent/
* *JSON-LD Context*:
https://palimpsestlicense.org/cwcf/v1/context.jsonld
